...
async function handleSave() {
  ...
  try {
    ...
    markSaved()
    showToast('success', '已提交，等待 GitHub Actions 构建...', 2500)
  ...
  <Teleport to="body">
    ...
    >
      <div class="w-full max-w-3xl mx-4 min-h-[560px] max-h-[85vh] flex flex-col bg-[--card-background-color] rounded-2xl shadow-xl overflow-hidden">
        ...
        <!-- 环境变量未配置提示 -->
        <div v-if="missingEnv" class="flex flex-col items-center justify-center flex-1 gap-3 text-[--sub-font-color]">
    ...
    <div class="fixed bottom-6 right-6 z-[60] flex flex-col gap-2 pointer-events-none">
      <TransitionGroup name="toast">
        <div
          ...
          :key="toast.id"
          class="flex items-center gap-3 px-4 py-3 rounded-xl shadow-lg text-sm pointer-events-auto min-w-[220px] max-w-sm"
          :class="{
            'bg-green-500 text-white': toast.type === 'success',
            'bg-red-500 text-white': toast.type === 'error',
            'bg-[--card-background-color] text-[--font-color] border border-[--sub-font-color]/20': toast.type === 'info',
          }"
        >
          <svg v-if="toast.type === 'success'" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="shrink-0"><circle cx="12" cy="12" r="10"/><path d="M8 12l3 3 5-5"/></svg>
          <svg v-else-if="toast.type === 'error'" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="shrink-0"><circle cx="12" cy="12" r="10"/><path d="M12 8v4m0 4h.01"/></svg>
